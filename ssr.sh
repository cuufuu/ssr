...
server_file="/etc/shadowsocksr/shadowsocks/server.py"
...
EOF
	...
	if [[ ${release} = "centos" ]]; then
		...
		sed -i '/nohup python \/etc\/shadowsocksr\/shadowsocks\/server.py a >> ssserver.log 2>&1 &/d' /etc/rc.d/rc.local
		echo -e "nohup python ${server_file} a >> ssserver.log 2>&1 &" >> /etc/rc.d/rc.local
	else
		chmod +x /etc/rc.local
		sed -i '$d' /etc/rc.local
		sed -i '/nohup python \/etc\/shadowsocksr\/shadowsocks\/server.py a >> ssserver.log 2>&1 &/d' /etc/rc.local
		echo -e "nohup python ${server_file} a >> ssserver.log 2>&1 &" >> /etc/rc.local
		echo -e "exit 0" >> /etc/rc.local
	fi
	#启动SSR服务端，并判断是否启动成功
	nohup python ${server_file} a >> ssserver.log 2>&1 &
	sleep 2s
	PID=`ps -ef |grep -v grep | grep server.py |awk '{print $2}'`
	if [ ! -z "${PID}" ]; then
		viewUser
...
#查看 ShadowsocksR 日志
TailSSR(){
	[ ! -f ${ssr_file}"/shadowsocks/ssserver.log" ] && echo -e "\033[41;37m [错误] \033[0m 没有发现ShadowsocksR日志文件，请检查 !" && exit 1
	echo
	echo -e "使用 \033[41;37m Ctrl+C \033[0m 键退出查看日志 !"
	echo
	cd ${ssr_file}"/shadowsocks"
	tail -f ssserver.log
}
#查看 ShadowsocksR 状态
StatusSSR(){
...
installServerSpeeder(){
	[ -f "/serverspeeder" ] && echo -e "\033[41;37m [错误] \033[0m 锐速(ServerSpeeder) 已安装 !" && exit 1
	cd /root
	#借用91yun.rog的开心版锐速
...
UninstallServerSpeeder(){
	[ ! -f "/serverspeeder" ] && echo -e "\033[41;37m [错误] \033[0m 锐速(ServerSpeeder) 没有安装，请检查 !" && exit 1
...
echo "————————————————"
echo -e "注意：BBR 不支持OpenVZ虚拟化VPS !"
echo -e "\033[42;37m 23. \033[0m 安装 BBR(需更换内核, 存在风险)"
echo -e "\033[42;37m 24. \033[0m 封禁 BT/PT/垃圾邮件(SPAM)"
echo -e "\033[42;37m 25. \033[0m 设置 ShadowsocksR定时重启"
echo "————————————————"
